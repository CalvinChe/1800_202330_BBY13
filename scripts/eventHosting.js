...
//----------------------------------------------------------
function getHostings(user) {
    db.collection("users").doc(user.uid).get().then(userDoc => {
        // Get the Array of bookmarks
        let events = userDoc.data().events;
        // console.log(bookmarks);

        // Get pointer the new card template
        let hostingCardTemplate = document.getElementById("hostingCardTemplate"); // Retrieve the HTML element with the ID "hikeCardTemplate" and store it in the cardTemplate variable. 

        // Iterate through the ARRAY of bookmarked hikes (document ID's)
        events.forEach(thisEventID => {
            ...
            db.collection("events").doc(thisEventID).get().then(doc => {  //iterate thru each doc
                ...
                let docID = doc.id;
                let newcard = hostingCardTemplate.content.cloneNode(true); // Clone the HTML template to create a new card (newcard) that will be filled with Firestore data.
